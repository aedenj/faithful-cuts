...
_EXAMPLES_TEMPLATE_task1 = string.Template("""
$query_name: $query
$input_name: $input
$output_name: $output""".strip())

_TEST_TEMPLATE_task1 = string.Template("""
$query_name: $query
...
_TUPLE_PREAMBLE = """Task: given input prompts and queries, describe each scene with skill-specific tuples. 
The described scene is based on the actual content of a video. Do not generate any tuples that contain information inferred beyond what is explicitly depicted in the video. For example, if the video shows a person standing on a car while sewing, and the question asks about unusual aspects of the scene, the response may mention that people usually sew indoors. However, in this case, the tuples should only describe what is directly observed in the video (e.g., 'person standing on car', 'person sewing'), while the usual scenario ('person sewing indoors') should not be included in the generated tuples. Ensure that the generated tuples strictly reflect what is visible and occurring within the video itself, avoiding any additional speculative or associative content.  
You must generate tuples in the response for the descriptive content of the input video.
Extract tuples only for the descriptive parts of the content. If the content includes both descriptive and inferential elements, generate tuples only for the descriptive facts. For inference-based statements that are partially grounded in observable facts, generate tuples based on the factual components.
...
Please note the numbers of questions should be same as the numbers of tuples to ensure every tuple has a corresponding question.
...
def tifa_id2example(
    ...
    # print(df[df.item_id == id].query)
    prompt = df[df.item_id == id].text.tolist()[0]
    query = df[df.item_id == id].input_query.tolist()[0]
...
assert len(TIFA160_ICL_TRAIN_IDS) == 26, len(TIFA160_ICL_TRAIN_IDS)
...
def make_prompt(
    ...
    for example in examples:
        if task == "tuple":
            examples_str.append(
    		_EXAMPLES_TEMPLATE_task1.substitute(
    ...
    examples_str = "\n\n".join(examples_str)
    if task == "tuple":
        test_input_str = _TEST_TEMPLATE_task1.substitute(
        input_name=input_name,
        output_name=output_name,
...
def generate_with_in_context_examples(
    ...
    for id_ in tqdm.tqdm(
        ...
        test_query = id2inputs[id_]["query"]
    ...
    for i, id_ in enumerate(
        ...
        out_datum["query"] = id2inputs[id_]["query"]
